...
class Order_model extends My_model
{
    ...
    {
        $data['table'] = 'order as o';
        $data['select'] = ['o.shopping_amount_based_discount', 'o.payable_amount', 'o.sub_total', 'o.order_status', 'o.order_no', 'o.delivery_charge', 'op.percentage', 'op.amount as promocodeAmount'];
    ...
    public function user_sell_report()
    {
        ...
        $data['where']['o.order_status!='] = '9';
        $data['select'] = [
            'o.branch_id', 'u.fname', 'u.lname', 'u.email', 'o.payable_amount', 'o.order_no',
    ...
    // DIpesh Added
    public function getUpdatedOrder($order_id)
    {
        $branch_id = $this->session->userdata['id'];
        ...
        $data['select'] = [
            ...
            '(od.quantity -  IFNULL(r_od.quantity, 0)) as updated_quantity',
            'od.dt_updated',
            'p.name',
        ...
        $data['where'] = ['id' => $order_id, 'branch_id' => $branch_id, 'status!=' => '9'];

        $r = $this->selectRecords($data);

        return ['result' => $result, 'info' => $r];
    }
    // 
}
