...
class staff_api_model extends my_model {
    function login($postdata) {
        ...
        $data['select'] = ['s.id', 's.vendor_id', 's.name', 's.email', 's.email_verify', 's.email_token', 's.phone_no', 'v.name AS shopname', 'v.location', 's.status', 'v.delivery_by'];
        $data['where'] = ['s.email' => $email, 's.password' => $password];
        $data['table'] = 'staff AS s';
        $data['join'] = ['vendor  AS v' => ['v.id = s.vendor_id', 'LEFT', ]];
    ...
    function order_list($postdata) {
        ...
        $data['where'] = ['o.vendor_id' => $vendor_id, 'o.status !=' => '9'];
    ...
    function product_check($postdata) {
        ...
        if ($res == $res2) {
            ...
            $data['join'] = ['vendor as v' => ['o.vendor_id = v.id', 'LEFT'], ];
