      ...
      <div class="row">
        <div class="col-sm-6">
          <div id="client">
            ...
            <div class="row">
              ...
              <div class="col-sm-6 email"><span><a href="mailto:john@example.com"><?php echo $order_detail_result[0]->email; ?></a></span>
              </div>
            </div>
        ...
        <div class="col-sm-6">
          <div id="company">
            ...
            <div><a href="mailto:<?php echo $vendor_detail->email; ?>"><?php echo $vendor_detail->email; ?></a>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="table-responsive-sm invoice-table mt-4 mb-5">
        <table border="0" cellspacing="0" cellpadding="0">
          ...
          <tbody>
            ...
            foreach ($updatedOrder as $key => $v) {
              if ($v->updated_quantity > 0) {
                $noQuant = false;
                $gstOne = numberFormat(numberFormat($v->discounted_price) - numberFormat(numberFormat($v->discounted_price) * $v->gst / 100));
                $sub_total += numberFormat($gstOne * $v->updated_quantity);
            ?>
                <tr>
                  <td><?= $v->name ?></td>
                  ...
                  <?php
                  if ($v->discount > 0) {
                    $discountAmt = numberFormat(numberFormat($v->actual_price) * $v->discount / 100);
                  }
                  if ($v->gst > 0) {
                    $total_gst_amount += numberFormat(numberFormat($v->discounted_price) * $v->gst / 100) * $v->updated_quantity;
                  }
                  if ($other_disc > 0) {
                    $val = numberFormat($v->discounted_price - numberFormat(numberFormat($v->discounted_price) * $other_disc / 100));
                    $total_discounted_gst += numberFormat(numberFormat($val * $v->gst / 100) * $v->updated_quantity);
                  }
                  ?>
                  ...
                  <td><?= isset($v->gst) ? '(' . $v->gst . '%)' . numberFormat(numberFormat($v->discounted_price) * $v->gst / 100)  : '-' ?>
                  </td>
                  <td>
                    <?= $currency . ' ' ?>
                    <span>
                      <?= $isShow ? numberFormat(numberFormat($v->without_gst_price) * $v->updated_quantity) :  numberFormat(numberFormat($v->discounted_price) * $v->updated_quantity) ?></span>
                  </td>
                </tr>
            <?php
              }
          ...
          <tfoot>
            ...
            <?php
            if (isset($amount) && $amount->percentage > 0) {
              $disc = numberFormat(numberFormat($sub_total) * $amount->percentage / 100);
            ?>
              <tr>
                <td colspan="4"></td>
            ...
            if (isset($shoppingDiscount) && $shoppingDiscount > 0) {
              $disc = numberFormat(numberFormat($sub_total) * $shoppingDiscount / 100);
            ?>
            ...
            <tr>
              ...
              <td><strong><?php
                          if ($total_discounted_gst > 0) {
                            $total = (numberFormat($sub_total) + numberFormat($total_discounted_gst)) -  numberFormat($disc);
                            echo $currency . ' ' . numberFormat($total);
                          } else {
                            if (isset($disc) && $disc > 0) {
                              echo $currency . ' ' . numberFormat(numberFormat(numberFormat($sub_total) + numberFormat($total_gst_amount)) - numberFormat($disc));
                            } else {
                              echo $currency . ' ' . numberFormat($sub_total + numberFormat($total_gst_amount));
                            }
                          }
                          ?> </strong></td>
            </tr>
