    ...
    <section class="wrapper">
        ...
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <!--breadcrumbs start -->
                <ul class="breadcrumb">
                    <li class="active"><a href=""><i class="fa fa-home"></i> <a href="<?php echo base_url().'admin/index'; ?>">Home</a> / Package</a></li>
                </ul>
                <!--breadcrumbs end -->
            </div>
        </div>
                        ...
                        <div class="adv-table">
                            <div id="example_wrapper" class="dataTables_wrapper form-inline" role="grid">
                                <div class="panel-body padding-zero" style="float: right">
                                    <a href="<?php echo base_url() . 'package/package_profile'; ?>" class="btn btn-primary">Add Package</a>
                                    <a href="#" id="delete_user" class="btn btn-danger">Delete Package</a>
                                </div>
                                <table class="display table table-bordered table-striped dataTable" id="example_package"
                                   ...
                                   <!--  <tbody role="alert" aria-live="polite" aria-relevant="all">
                                    <?php foreach ($package_results as $result){ ?>
                                        <tr class="gradeX odd">
                                            ...
                                            <td class="hidden-phone"><?php echo $result->package; ?></td>
                                             <td>
                                                <a href="<?php echo base_url() . 'package/package_profile?id=' . $this->utility->encode($result->id); ?>" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>
    ...
    function single_delete(value) {
        ...
        bootbox.confirm("Are you sure you want to delete ?" , function (confirmed) {
            if (confirmed == true) {
                var id = value;
                $.ajax({
                    url: '<?php echo base_url().'package/single_delete_package'; ?>' ,
                    data: {
                        id: id
                    },
                    success: function (data) {
                        ...
                        if (data.status == 1) {
                            bootbox.alert("Package has been deleted successfully.", function() {
                    ...
                    error: function () {
                        alert('Failed to delete selected package.');
                    }
                });
            }
            else
    ...
    $('#delete_user').click(function() {
        ...
        bootbox.confirm("Are you sure you want to delete ?" , function (confirmed) {
            if (confirmed == true) {
                ...
                $.ajax({
                    url: '<?php echo base_url().'package/multi_delete_package'; ?>',
                    data: { ids: ids.toString() },
                    success: function(data) {
                        if(data.status == 1) {

                            bootbox.alert("Package(s) has been deleted successfully.", function() {
                                window.location.reload(true);
                            });
                        }
                        else {
