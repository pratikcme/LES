...
<section class="new-customer background-blue-grey">
    ...
    <div class="container">
        <div class="row">
            <form id="Form" method="POST" action="<?= $FormAction ?>" class="new-cust-form basic-form"
                enctype='multipart/form-data'>
                <div class="invoice-title border-grey">
                    <h3 class="mb-0"><span><i class="far fa-file-alt"></i></span> Select Store Type For Food Category
                    </h3>
                </div>
                ...
                <div class="row mt-5">
                    <div class="col-lg-6">
                        <div class="form-group">
                            <label for="store_in">All Store Types</label>
                            <div class="row">
                                <div class="col-sm-12">
                                    <select name="store_type[]" id="store_type" class="form-control"
                                        style="position: absolute; left: -9999px;" multiple='multiple'>
                                        <?php foreach ($store_type as $key => $value) { ?>
                                        <option value="<?= $value->id ?>"
                                            <?= (in_array($value->id, $foodCategory)) ? 'selected' : '' ?>>
                                            <?= $value->name ?></option>
                                        <?php } ?>
                                    </select>
