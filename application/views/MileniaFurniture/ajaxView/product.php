...
<div class="product-listing-wrapper hot-products-wrap wow fadeInDown <?= ($value->quantity == '0') ? 'out-of-stock' : '' ?>" data-wow-duration="1s" data-wow-delay="0" data-wow-offset="0">
    <div class="hot-products-img position-relative overflow-hidden ">
        ...
        <div class="hot-products-cart-wrap">
            <a href="javascript:;" class="addcartbutton <?= $d_none ?>" data-product_id="<?= $this->utility->safe_b64encode($value->id) ?>" data-varient_id="<?= $this->utility->safe_b64encode($value->product_weight_id) ?>"><svg width="22" height="20" viewBox="0 0 22 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <a href="<?= base_url() . 'products/productDetails/' . $this->utility->safe_b64encode($value->id) . '/' . $this->utility->safe_b64encode($value->product_weight_id) ?>"><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="techno-check">
                <input class="techno_checkbox wishlist-icon" type="checkbox" data-product_id="<?= $this->utility->safe_b64encode($value->id) ?>" data-product_weight_id="<?= $this->utility->safe_b64encode($value->product_weight_id) ?>" <?= (in_array($value->product_weight_id, $wish_pid)) ? "checked" : "" ?>>
