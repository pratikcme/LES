...
function getUsersList($TableData)
{
  ...
  foreach ($fetch_data as $row) {
    ...
    $sub_array[] = $row->lname;
    $sub_array[] = $row->email;
    $sub_array[] = $row->country_code . " " . $row->phone;
    $sub_array[] = date('Y/m/d H:i', $row->dt_added);
    // $sub_array[] = '<a href = the url>"' . $row->id . '"</a>';
    $sub_array[] = '<a href="javascript:;" 
    onclick="single_delete(' . $row->id . ')"
    class="btn btn-danger btn-xs deleteUser"><i class="fa fa-trash-o "></i></a>
    <a href="' . base_url() . 'admin/edit_user/'  . $CI->utility->encode($row->id) .
      '"
class="btn ml-4 btn-primary btn-xs editUser"><i class="fa fa-pencil"></i></a>';
?>
<?php
    // data-id="' . $row->id . '"
    $row->id;
