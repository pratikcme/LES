...
class Order extends Vendor_Controller
{
    function __construct()
    {
        ...
        $this->load->model('sell_development_model', 'sell_development_model');
    }
    ...
    public function order_detail()
    {
        ...
        // $postdata = ['order_id' => $this->utility->safe_b64encode($order_id)];

        // $re = $this->sell_development_model->viewOrderDetails($postdata);
        $r = $this->this_model->getUpdatedOrder($order_id);
        $data['updatedOrder'] = $r['result'];
        ...
        $currency = $this->sell_development_model->getCurrency();
        $data['currency'] = $currency[0]->value;

        $data['order_total_gst'] = 0;

        $data['orderInfo'] = $r['info'][0];
