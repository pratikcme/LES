...
class Products extends User_Controller
{
	...
	public function productDetails($id = '')
	{
		...
		foreach ($varient_ids as $key => $value) {
			...
			$varient_image[] = $product_image[0]->image;
		}
	...
	public function getDataProductWeight()
	{
		...
		$response = [
			...
			'varientWishStarRatting' => $starHtml . '<div><span>(' . count($data['product_review']) . ')</span></div>',
			'upbasket_starHtml' => '<div class="rating stars3_5" >' . $upbasket_starHtml . '</div><div><span>(' . count($data['product_review']) . ')</span></div>',
			// $upbasket_starHtml . '  ' . count($data['product_review']),
			'beauty_starHtml' => '<div class="rating stars3_5" >' . $beauty_starHtml . '</div><div><span>(' . count($data['product_review']) . ')</span></div>',
			'upbasket_thumb' => $upbasket_thumb,
			'shoppy_starHtml' => '<div class="rating stars3_5" >' . $shoppy_starHtml . '</div><div><span>(' . count($data['product_review']) . ')</span></div>',
			'furniture_starHtml' => '<div class="rating stars3_5">' . $shoppy_starHtml . '</div><div><span>(' . count($data['product_review']) . ')</span></div>',
