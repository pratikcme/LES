...
ALTER TABLE `branch` CHANGE `delivery_time_date` `delivery_time_date` ENUM('0','1') CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL DEFAULT '1' COMMENT '0=\"disable\",\"1\"=>enable';*/
    ...
    public function edit_user($id)
    {
        ...
        $data['init'] = array('USER.editUser()');
        $data['country_code'] = GetDialcodelist();
        $data['formAction'] = 'admin/user_update';
        $data['editData'] = $this->vendor_model->get_current_user($userId);
    ...
    public function user_update()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($validation) {
                ...
                $re = $this->this_model->updateUser($postdata);
                if ($re) {
                    $this->utility->setFlashMessage('success', 'Successfully updated user.');
