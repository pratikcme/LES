...
$(document).on('click', '#btnSubmit', function () {
  ...
  $.ajax({
    ...
    type: 'post',
    async: false,
    data: { product_id: product_id, varient_id: varient_id, ratetIndex: ratetIndex, comment: comment },
    dataType: 'json',
    ...
    if (product_varient_id != '') {
      ...
      $.ajax({
        ...
        success: function (output) {
          ...
          if (output.isVarientExist == 0 || output.countParticularUserReview >= 1) {
          ...
          disNwislist = disNwislist + '<div class="pro-hearticon wishlist-icon" data-product_id="' + output.product_id + '" data-product_weight_id="' + output.product_variant_id + '"><i class="fa-regular fa-heart ' + output.isInWishList + '"></i></div>';
          $(".discnt .pro-hearticon").remove();
          ...
          if (output.cartProductQuantity == 0) {
            ...
            that.next('div').find('.product-detail-quentity').addClass('d-none');
            that.next('div').find('.product-detail-quentity .qty-container .qty-btn-minus').attr('data-product_weight_id', output.product_weight_id);
            that.next('div').find('.product-detail-quentity .qty-container .qty-btn-plus').attr('data-product_weight_id', output.product_weight_id);
          } else {
            that.next('div').find('a:first').addClass('d-none');
            that.next('div').find('.product-detail-quentity').removeClass('d-none');
            that.next('div').find('.product-detail-quentity .qty-container .qty-btn-minus').attr('data-product_weight_id', output.product_weight_id);
            that.next('div').find('.product-detail-quentity .qty-container .qty-btn-plus').attr('data-product_weight_id', output.product_weight_id);
