...
class basictable {
  constructor(tableSel, options = {}) {
    ...
    if (
      ...
      this.options.containerBreakpoint === null
    ) {
    ...
    this.tables.forEach((table) => {
      ...
      if (this.options.header) {
        ...
        table.querySelectorAll(format).forEach((heading) => {
          ...
          const row = [].indexOf.call(
            parentRow.parentElement.children,
            parentRow
          );
          if (!headings[row]) {
            headings[row] = [];
        ...
        table
          ...
          .forEach((row) => this._setupRow(row, headings));
        table
  ...
  _setupRow(row, headings) {
    row.querySelectorAll(":scope > *").forEach((cell) => {
      if (
        (cell.innerHTML.trim() === "" || cell.innerHTML === "&nbsp;") &&
        this.options.showEmptyCells
      ) {
  ...
  _check(table) {
    if (!this.options.forceResponsive) {
      ...
      if (
        ...
        (this.options.containerBreakpoint !== null &&
          table.parentElement.offsetWidth <= this.options.containerBreakpoint)
      ) {
        this._start(table);
      } else {
        this._end(table);
